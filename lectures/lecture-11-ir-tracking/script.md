...
| VCC | 电源 | 3.3V-5V |
| GND | 地 | 共地 |
| OUT1 | 左路数字输出 | 比较器输出，0 或 1（接单片机） |
| OUT2 | 右路数字输出 | 比较器输出，0 或 1（接单片机） |
...
| 传感器 | 引脚 | 说明 |
| :--- | :--- | :--- |
| 左循迹 | P3^5（OUT1） | 检测左侧黑线 |
| 右循迹 | P3^4（OUT2） | 检测右侧黑线 |
...
s_l = left_track;      // 左传感器：0=白线，1=黑线
s_r = right_track;     // 右传感器：0=白线，1=黑线
...
```c
// 读传感器状态（示例代码）
unsigned char s_l = left_track;   // 读左传感器
unsigned char s_r = right_track;  // 读右传感器
```
...
if (state == 11) {
    ...
    right_run(80, 160);          // 左黑右白 → 右转纠偏
}
else if (state == 1) {
    left_run(160, 80);           // 左白右黑 → 左转纠偏
}
else {
    stop();                      // 两线都离线 → 停车
}
...
{
    // 1. 读取左右传感器
    unsigned char s_l = left_track;   // 左：0=白线，1=黑线
    unsigned char s_r = right_track;  // 右：0=白线，1=黑线
    ...
    else if (state == 10) {    // 左黑右白 → 右转纠偏
        right_run(80, 160);
    }
    else if (state == 1) {     // 左白右黑 → 左转纠偏
        left_run(160, 80);
    }
    else {                     // 两线都离线 → 停车
        stop();
