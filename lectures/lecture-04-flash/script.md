...
### ⑤ 软件链接&执行烧录：STC-ISP  
>打开烧录软件之前，先确保系统能识别到烧录器（USB能识别到串口）  
...
下节课我会给大家具体讲讲整个流程的原理，怎么做的，为什么这么做  
并且真正烧录一个有功能的程序进去  
...
>[!IMPORTANT]  
>“在 Dev C++ 里，你是在 Windows 上写程序给 Windows 跑；  
>但在 Keil 里，你是在 Windows 上写程序给 51 单片机（一种完全不同的 CPU 架构）跑。  
>这种在一个平台上为另一个平台生成代码的行为，就叫交叉编译
